...
create table practica.grupos_empresariales(
...
alter table practica.polizas add constraint fk_poliza_aseguradora foreign key (id_aseguradora) references practica.aseguradoras(id);
alter table practica.polizas add constraint fk_poliza_choches foreign key (matricula) references practica.coches(matricula);
alter table practica.marcas add constraint fk_marca_ge foreign key (id_ge) references practica.grupos_empresariales(id);
alter table practica.modelos add constraint fk_modelo_marca foreign key (id_marca) references practica.marcas(id);
alter table practica.revision add constraint fk_revision_coches foreign key (matricula) references practica.coches(matricula);
alter table practica.coches add constraint fk_coches_modelos foreign key (id_modelo) references practica.modelos(id);

CREATE TABLE practica.info(
...
INSERT INTO practica.info (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
insert into practica.aseguradoras (aseguradora) select aseguradora from practica.info group by aseguradora;
insert into practica.grupos_empresariales(grupo) select grupo from practica.info group by grupo;

insert into practica.marcas (marca,id_ge)
select 
	i.marca, 
	ge.id 
from practica.info i
inner join practica.grupos_empresariales ge on i.grupo = ge.grupo 
group by i.marca, ge.id
order by marca;

insert into practica.modelos (modelo,id_marca)
select 
	i.modelo, 
	ma.id 
from practica.info i
inner join practica.marcas ma on i.marca = ma.marca 
group by i.modelo, ma.id
order by modelo;

insert into practica.coches (matricula,id_modelo,fecha_compra,color,kms_totales)
select 
	i.matricula,
	mo.id,
	i.fecha_compra,
	...
	i.kms_totales
from practica.info i
inner join practica.modelos mo on i.modelo = mo.modelo
group by i.matricula, mo.id, i.fecha_compra, i.color, i.kms_totales;

insert into practica.revision (fecha_revision,kms_revision,importe_revision,moneda,matricula)
select
	i.fecha_revision,
	i.kms_revision,
	i.importe_revision,
...
insert into practica.polizas (n_poliza,id_aseguradora,fecha_alta_seguro,matricula)
select 
	i.n_poliza,
	a.id,
	i.fecha_alta_seguro,
	i.matricula
from practica.info i
inner join practica.aseguradoras a on i.aseguradora = a.aseguradora
group by i.n_poliza,
	i.fecha_alta_seguro,
	i.aseguradora,
	a.id,
...
drop table practica.info;

select mo.modelo, ma.marca, ge.grupo, c.fecha_compra, c.matricula, c.color, c.kms_totales, a.aseguradora, p.n_poliza
from practica.coches c
inner join practica.modelos mo on mo.id = c.id_modelo
inner join practica.marcas ma on ma.id = mo.id_marca
inner join practica.grupos_empresariales ge on ge.id = ma.id_ge
inner join practica.polizas p on p.matricula = c.matricula
inner join practica.aseguradoras a on a.id = p.id_aseguradora 
;
